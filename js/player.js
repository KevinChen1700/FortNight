...
var Player = function() {
    ...
    this.update = function(dt) {
        if (((key.down(key.FORWARD) || key.down(key.ARROW_FORWARD))) && this.speedboost == false) {
            // Forward movement
        ...
        else if(((key.down(key.FORWARD) || key.down(key.ARROW_FORWARD))) && this.speedboost == true && this.stamina > 10) 
        {
            if (this.velocity.z > -this.maxVelocity) {
                this.velocity.z = -this.speed * 2;
                this.stamina -= 10;
            }
        } 
        ...
        //crouch toggle
        if(key.down(key.CROUCH))
        {
            camera.position.set(0, -0.25, 0); 
            this.CubeGeometry =(0.02, 0.01, 0.02);          
        }
        else
        {
            camera.position.set(0, 0, 0); 
            this.CubeGeometry =(0.02, 0.02, 0.02); 
        }
        ...
        //new one for crouch
        var currentPositionY = controls.getObject().position.y;
        ...
        for (var x = 0; x < map.length; x++) {
            for (var z = 0; z < map[x].length; z++) {
                switch (map[x][z]) {
                    case Tile.WALL:
                    //case Tile.GATE:// diabled to walk trough doors
